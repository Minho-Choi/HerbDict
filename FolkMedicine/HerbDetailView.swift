...
struct HerbDetailView: View {
    ...
    var id: String
    @ObservedObject var herbDetailData = getDetail()
    
    var body: some View {

        ZStack{
            HerbDetail(herbData: herbDetailData.herbData)
            .onAppear {
                self.herbDetailData.getDtlHerb(cntntsNo: self.id)
            }
            LoadingView()
        .opacity(showSplash ? 1 : 0)
        .onAppear {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                LoadingView.shouldAnimate = false
                withAnimation() {
...
struct herbDetail: Codable {
    ...
    var ImgUrl: [String] = [String](repeating: "0", count: 6)
...
class getDetail: ObservableObject {
    ...
    @Published var herbData: herbDetail = herbDetail.init()
        ...
        .responseString { response in
            ...
            case .success:
                if let string = response.value {
                    ...
                    if let imgUrl1 = element["imgUrl1"].text {
                        self.herbData.ImgUrl[0] = imgUrl1
                        print(imgUrl1)
                    }
                    if let imgUrl2 = element["imgUrl2"].text {
                        self.herbData.ImgUrl[1] = imgUrl2
                        print(imgUrl2)
                    }
                    if let imgUrl3 = element["imgUrl3"].text {
                        self.herbData.ImgUrl[2] = imgUrl3
                        print(imgUrl3)
                    }
                    if let imgUrl4 = element["imgUrl4"].text {
                        self.herbData.ImgUrl[3] = imgUrl4
                        print(imgUrl4)
                    }
                    if let imgUrl5 = element["imgUrl5"].text {
                        self.herbData.ImgUrl[4] = imgUrl5
                        print(imgUrl5)
                    }
                    if let imgUrl6 = element["imgUrl6"].text {
                        self.herbData.ImgUrl[5] = imgUrl6
                        print(imgUrl6)
                    }
                }
                print(self.herbData.ImgUrl)
            case .failure(_):
                print("url failure")
            }
        }
